...
parser.add_argument("--build", help="Build the projects found in destdir", action="store_true")
parser.add_argument("--clean", help="Clean before build the projects found in destdir", action="store_true")
...
class Archive(object):
    ...
    def get_toplevelDir(self):
        self.compute_toplevel_dir()
        return self.toplevelDir
    ...
    def check(self) -> bool:
        ...
        for f in self.filePaths:
            if f.is_absolute() or f.parts[0] == '..':
                print(f"\t{f.as_posix()} refers to a file outside the toplevel directory")
                check = False
...
def copy_execs(project):
    ...
    for e in EXECS:
        ...
        if exec_path.exists():
            ...
            print(f"\tCopying {e} to {dest_exec.as_posix()}")
...
if __name__ == "__main__":
    ...
    for arg in args_list:
        ...
        if check:
            ...
            print("Checking archive format... ", "OK" if archive_isvalid else "FAIL")
        if extract:
            print(f"Extracting {arg} to {destdir}")
            archive.extract()
    if build:
        for d in destdir.iterdir():
            if not d.is_dir():
                continue
            project = d.name
            print(f"-- Building {project}...")
            is_built = build_project(project)
